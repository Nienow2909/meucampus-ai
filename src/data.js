...
export async function catalog(){return supabase ? await unwrap(supabase.from('universities').select('id,name,country,city,courses,cycle,catalog_rank,institutional_group').order('catalog_rank',{nullsFirst:false}).order('name').abortSignal(AbortSignal.timeout(15000))) : [];}
